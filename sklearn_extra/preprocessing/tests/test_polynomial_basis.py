...
def test_correct_param_types():
    ...
    with pytest.raises(ValueError):
        BernsteinFeatures(degree=1, bias=1, na_value=1, interactions=1).fit(feature_1d)

    with pytest.raises(ValueError):
        BernsteinFeatures(degree=1, bias=1, na_value=1, interactions=1.5).fit(feature_1d)

    with pytest.raises(ValueError):
        BernsteinFeatures(degree=1, bias=1, na_value=1, interactions=1, unknown=1).fit(feature_1d)
